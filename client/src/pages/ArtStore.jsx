    ...
    // NEW: State for Category Filtering (Default is "Artwork")
    const [activeCategory, setActiveCategory] = useState("Artwork");
    ...
    if (loading) {
        ...
        <div className="min-h-screen flex items-center justify-center pt-20">
            ...
            <p className="text-black font-bold">Curating Store...</p>
            </div>
        </div>
        );
    }
    ...
    // --- NEW: FILTER LOGIC ---
    const filteredItems = artPieces.filter(item => {
        if (activeCategory === "All") return true;
        if (activeCategory === "Stationary") return item.category === "Stationary";
        if (activeCategory === "Craft") return item.category === "Craft";
        
        // If "Artwork" is selected, show everything that ISN'T Stationary or Craft
        if (activeCategory === "Artwork") {
        return item.category !== "Stationary" && item.category !== "Craft";
        }
        return true;
    });

    const categories = ["All", "Artwork", "Stationary", "Craft"];
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="glass-card p-10 md:p-14 rounded-[3rem] text-center max-w-4xl border border-white/20 bg-white/10 backdrop-blur-xl shadow-2xl relative overflow-hidden group">
                ...
                Original pieces, custom stationary, and handcrafted items made with love.
                </p>
            </div>
            </div>
            ...
            {categories.map((cat) => (
                ...
                className={`px-6 py-2 rounded-full font-bold transition-all duration-300 ${
                    activeCategory === cat
                    ? "bg-[#D984B5] text-white shadow-lg shadow-[#D984B5]/40"
                    : "bg-white/20 text-black hover:bg-white/40 border border-black/10"
                }`}
                >
                {cat}
                </button>
            ...
            <div className="text-center text-black text-xl mt-20 font-medium">
            ...
            <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
                ...
                >
                    ...
                    style={{ aspectRatio: `${art.width || 400} / ${art.height || 400}` }}
                    >
                    <img
                        src={art.image}
                        alt={art.title}
                    ...
                    {/* Details */}
                    <div className="p-6">
                    <h3 className="text-xl font-bold text-black mb-2 group-hover:text-[#ff9ed2] transition">
                        {art.title}
                    </h3>
                    ...
                    >
                        <ShoppingBag size={18} />
                        {art.inStock ? "Add to Cart" : "Out of Stock"}
                    </button>
                    </div>
                </div>
                ))}
